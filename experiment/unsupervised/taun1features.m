    ...
    case {'mfcc', 'scatT'}
        ...
        switch setting.dataset
            ...
            case {'2016', '2017'}
                ...
                for k=1:length(typo)
                    info = strsplit(store.xp_settings.sounds{k}(7:end), '_');
                    store.xp_settings.recIndex{k} = info{1};
        ...
        for jj=1:length(store.xp_settings.soundIndex)
            ...
            [signal,fs]=audioread([config.inputPath 'dcase' setting.dataset(1:4) '/' store.xp_settings.sounds{store.xp_settings.soundIndex(jj)}  '.wav']);
